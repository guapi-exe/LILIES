...
function reload_weapons(name){
    load_info(`http://127.0.0.1:8099/api/bf2042info?name=${name}`).then(data=>{
        console.log(data)
        let weapons_html = ``
        for(let weapon in data["weapons"]){
            let name = data["weapons"][weapon]["weaponName"]
            let type = data["weapons"][weapon]["type"]
            let kills = data["weapons"][weapon]["kills"]
            let kpm = data["weapons"][weapon]["killsPerMinute"]
            let image = data["weapons"][weapon]["image"]
            let accuracy = data["weapons"][weapon]["accuracy"]
            let headshots = data["weapons"][weapon]["headshots"]
            let timeEquipped = data["weapons"][weapon]["timeEquipped"]
            weapons_html +=
                `<div class="column">
                        <div class="row">
    ...
    <div class="list">
        ...
        let avatar = data["row"]["avatar"]
        let rank = data["row"]["level"]
        let rankImg = data["row"]["rankImg"]
        let accuracy = data["row"]["accuracy"]
        let killsPerMinute = data["row"]["killsPerMinute"]
        let winPercent = data["row"]["winPercent"]
        let headshots = data["row"]["headshots"]
        let secondsPlayed = data["row"]["secondsPlayed"]
        let killDeath = data["row"]["killDeath"]
        let kills = data["row"]["kills"]
        let deaths = data["row"]["deaths"]
        let wins = data["row"]["wins"]
        let revives = data["row"]["revives"]
        let mvp = data["row"]["mvp"]
        let header =
            `<div data-v="" class="flex">
        <div data-v="" class="title" >
            <div data-v="" class="flex flex-col">
    ...
    <div data-v="" class="main" >
        ...
        <div data-v-1="" data-v="" class="stat align-left expandable">
            <div data-v-1="" class="wrapper">
                <div data-v-1="" class="numbers">
                    <span data-v-1="" class="name" title="Score/Min">MVP(连杀)</span>
                    <span data-v-1="" class="flex items-center gap-2">
                        <span data-v-1="" class="value">${mvp}</span>
                    </span>
                </div>
            </div>
        </div>
        ...
        let vehicles_html = ``
        for(let weapon in data["vehicles"]){
            let name = data["vehicles"][weapon]["vehicleName"]
            let type = data["vehicles"][weapon]["type"]
            let kills = data["vehicles"][weapon]["kills"]
            let kpm = data["vehicles"][weapon]["killsPerMinute"]
            let image = data["vehicles"][weapon]["image"]
            let destroyed = data["vehicles"][weapon]["destroyed"]
            let timeIn = data["vehicles"][weapon]["timeIn"]
            vehicles_html +=
                `<div class="column">
                        <div class="row">
                            <h4 class="name">${name}</h4>
    ...
    <div class="list">
        ...
        let classes_html = ``
        for(let weapon in data["classes"]){
            let name = data["classes"][weapon]["className"]
            let kills = data["classes"][weapon]["kills"]
            let kpm = data["classes"][weapon]["kpm"]
            let image = data["classes"][weapon]["image"]
            let score = data["classes"][weapon]["killDeath"]
            let secondsPlayed = data["classes"][weapon]["secondsPlayed"]
            classes_html +=
                `<div class="column">
                        <div class="row">
                            <h4 class="name">${name}</h4>
